...
Trainer for amortized min-STP learning on gaussian to ModelNet10 classes.
"""
...
class AmortizedSlicerTrainer:
    ...
    def _pair_forward_from_noise(self, X, Y, cY, model, context_proj):
        """Forward pass returning projected embeddings and distance matrix."""
        Nx, _ = X.shape
        Ny, _ = Y.shape
        M = torch.sqrt(ot.dist(X, Y))
        ...
        Xinput = torch.cat([X, cy_proj.repeat(Nx, 1)], dim=-1)
        Yinput = torch.cat([Y, cy_proj.repeat(Ny, 1)], dim=-1)
        # Yinput = Y
    ...
    def train(self):
        ...
        for e in range(self.outer_epochs):
            ...
            for pc2 in tqdm(self.train_loader, desc=f"Epoch {e+1}/{self.outer_epochs}"):
                ...
                for _ in range(self.inner_epochs):
                    ...
                    Xout, Yout, M, _, _ = self._pair_forward_from_noise(X, Y, cY, model, context_proj)
            ...
            if e % 6 == 0 or e == self.outer_epochs - 1:
                ...
                torch.save(model.state_dict(), os.path.join(self.save_dir, "pointnet_model_ckpt.pth"))
                print("Model saved.")
    ...
    def evaluate(self, model, context_proj, loader):
        ...
        for pc2 in loader:
            ...
            Xout, Yout, M, a, b = self._pair_forward_from_noise(X, Y, cY, model, context_proj)
